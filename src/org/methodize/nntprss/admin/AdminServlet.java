...
// Currently used within posting/publishing configuration.
	private static final String PASSWORD_MAGIC_KEY
		= "###__KCV__###";
		...
		if(channel == null) {
			...
			if(channel.getManagingEditor() != null) {
				writer.write("<a href='mailto:"
					+ URLEncoder.encode(RSSHelper.parseEmail(channel.getManagingEditor()))
					+ "'>"
			...
			writer.write("</td></tr>");
			writer.write("<tr><td class='row1' align='right'>Posting</td><td class='row2'><select name='postingEnabled' onChange='this.form.action=\"?action=editchlrefresh\"; this.form.submit();'>"
				+ "<option " + (postingEnabled? "selected" : "") + ">true"
			...
			if(postingEnabled) {
				...
				writer.write("<tr><td class='row1' align='right'>API</td><td class='row2'><select name='publishAPI' onChange='this.form.action=\"?action=editchlrefresh&publishapichange=true\"; this.form.submit();'>"
					+ "<option value='blogger' " + (publishAPI == null || publishAPI.equals("blogger") ? "selected" : "") + ">Blogger"
					+ "<option value='livejournal' " + (publishAPI != null && publishAPI.equals("livejournal") ? "selected" : "") + ">LiveJournal"
					+ "<option value='metaweblog' " + (publishAPI != null && publishAPI.equals("metaweblog") ? "selected" : "") + ">MetaWeblog"
//					+ "<option " + (!postingEnabled ? "selected" : "") + ">false"
					+ "</select></td></tr>");
...
// If a initial channel view, extract parameter from the Channel publish config map
						...
						if(publishConfig != null) {
							...
							if(password != null) password = PASSWORD_MAGIC_KEY;
...
// Make sure that everything has a value, especially if publish has just been enabled
					if(publishUrl == null) publishUrl = "http://plant.blogger.com/api/RPC2";
					...
					writer.write("<td class='row2'><input name='"
...
// If a refresh, get the parameter values from the parameter collection
						...
						blogId = (String)request.getParameter(BloggerPublisher.PROP_BLOG_ID);
						if(blogId != null && blogId.length() == 0) blogId = "home";
...
// Make sure that everything has a value, especially if publish has just been enabled
					if(publishUrl == null) publishUrl = "http://127.0.0.1:5335/RPC2";
					if(blogId == null) blogId = "home";
					if(userName == null) userName = "";
					if(password == null) password = "";
...
// Make sure that everything has a value, especially if publish has just been enabled
					if(publishUrl == null) publishUrl = "http://www.livejournal.com/interface/xmlrpc";
					if(userName == null) userName = "";
					if(password == null) password = "";
...
//TODO: improve componentization / pluggability of publishers
				if(publishAPI.equals("blogger")) {
					...
					try {
						...
						errors.add("Error validating Blogger posting configuration - "
							+ pe.getMessage());
					...
					try {
						...
						errors.add("Error validating MetaWeblog posting configuration - "
							+ pe.getMessage());
					...
					try {
						...
						errors.add("Error validating LiveJournal posting configuration - "
							+ pe.getMessage());
...
//		response.setContentType("text/xml");
	...
	private void writeImportForm(Writer writer) throws IOException {
		...
		writer.write("<td class='row2'><input type='radio' name='type' value='nntp' checked>nntp//rss Channel List<br>"
...
// XXX Expectation is that there will be a body element
			Element bodyElm = (Element)docElm.getElementsByTagName("body").item(0);
			...
			if(channels.getLength() > 0) {			
				...
				for(int channelCount = 0; channelCount < channels.getLength(); channelCount++) {
					...
					writer.write("<tr>"
						+ "<td class='" + rowClass + "' align='center'><input type='checkbox' name='import"
						+ channelCount
						+ "' checked></td>"
						+"<td class='" + rowClass + "'><input type='value' size='50' name='name"
						+ channelCount
						+ "' value='"
	...
	// Check name...
				...
				if(currentErrors.size() > 0) {
					errorContent.append("<tr>"
						...
						+ HTMLHelper.escapeString(urlString)
						+ "</td></tr>\n");
					errorContent.append("<tr><td colspan='3'>&nbsp;&nbsp;<font color='red'>");
...
// Display any errors encountered during processing...
	...
	private void cmdHelp(
		...
		writer.write("<tr><td><br>Access to the web admin interface can be password protected by creating a file named users.properties in the root directory of your nntp//rss installation.<p>"
		...
		writer.write("<tr><td><br>Read the <a href='http://www.methodize.org/nntprss/docs/WINDOWS-SERVICE.TXT'>WINDOWS-SERVICE.TXT</a> file, located in the installation directory, for information on how to configure nntp//rss to run as a Windows service.<p>"
